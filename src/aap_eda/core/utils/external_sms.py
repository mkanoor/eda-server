...
def get_external_secrets(credential_id: int) -> dict:
    ...
    for obj in models.CredentialInputSource.objects.filter(
        ...
        try:
            if isinstance(obj.source_credential.inputs, SecretValue):
                inputs = obj.source_credential.inputs.get_secret_value()
            else:
                inputs = obj.source_credential.inputs
            ...
            LOGGER.error(msg)
            raise RuntimeErrorFetchingExternalSecret(msg)
    return result
