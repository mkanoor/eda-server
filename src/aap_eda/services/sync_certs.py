...
class SyncCertificates:
    ...
    def __init__(self, eda_credential_id: int, x_dab_jw_token: str):
        ...
        self.x_dab_jw_token = x_dab_jw_token
    ...
    def _prep_headers(self) -> dict:
        if self.x_dab_jw_token:
            return {"X-DAB-JW-TOKEN": self.x_dab_jw_token}
...
def gw_handler(sender, instance, **kwargs):
    ...
    if (
        ...
        try:
            x_dab_jw_token = instance._request.headers.get("x-dab-jw-token")
            objects = models.EventStream.objects.filter(
                eda_credential_id=instance.id
            )
            if len(objects) > 0:
                SyncCertificates(instance.id, x_dab_jw_token).update()
