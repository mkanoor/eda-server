        ...
        try:
            if self.client.containers.exists(container_id):
                ...
                if container.status == "exited":
                    PodmanReadLogs(
                        self.podman_url, self.activation_db_logger
                    ).run(activation_instance)
                    exit_code = container.wait(condition="exited")
                    logger.info(f"return_code: {exit_code}")
                    message = f"Container {container_id} has exited "
            ...
            logger.exception(
                f"Failed to get container status container: {container_id}; "
                f"error: {str(e)}"
            )
            raise
        return status
